...
/*
 * First, we need a list of servers (right?).
 * This list will be found our central web server.
 * For now, we'll just assume all the servers are bound to localhost.
 *
 * Next, we set up our server to listen for clients.
 *
 * Our client will ask for a "file"
 * Our servers, then, have to work together to stream different parts of the file
 */
